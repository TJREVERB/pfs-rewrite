...
class MainControlLoop:
    ...
    def iterate(self):  # Repeat main control loop forever
        ...
        print(f"Transmit buffer looks like this: {self.sfr.vars.transmit_buffer}", file = open("pfs-output.txt", "a"))  # TODO: DELETE THIS AFTER TESTING ICT
        ...
        if not self.sfr.vars.MODE_LOCK or self.sfr.check_lower_threshold():
            if not isinstance(self.sfr.MODE, type(new_mode := self.sfr.MODE.suggested_mode())):
                ...
                if not self.sfr.switch_mode(new_mode):
                    ...
                    print(f"Switch failed because of locked components! Staying in {self.sfr.MODE}", file = open("pfs-output.txt", "a"))
